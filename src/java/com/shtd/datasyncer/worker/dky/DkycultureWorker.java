...
import com.shtd.datasyncer.dbsaver.dky.dkyculture.DataSaveWorker;
 ...
 *
 */
public class DkycultureWorker implements IWorker {
	private static Logger logger = Logger.getLogger(Constant.LOGGER_NAME);
	
	public void work() {
		
		// 数据获取
		logger.info("dky dkyculture worker 工作开始，从webservice获取数据");
		...
		if ((Constant.INT_TRUE+"").equals(ConfigReader.getInstance().getValue("GET_DATA_ONLY"))) {
			logger.info("当前配置为只获取数据并存储到本地，dky dkyculture worker 工作结束。");
			return;
		}
		...
		logger.info("dky dkyculture worker 工作结束");
	}
}
